...
Separated from paper_trader to allow CLI, cron, and API usage.
"""
...
BINANCE_MONTHLY = f"{BINANCE_BASE}?symbol=BTCUSDT&interval=1M&limit={MONTHLY_LIMIT}"
BINANCE_WEEKLY = f"{BINANCE_BASE}?symbol=BTCUSDT&interval=1w&limit={WEEKLY_LIMIT}"
...
def fetch_data():
    """Fetch monthly and weekly BTC klines."""
...
def check_signal(strategy_class=None) -> dict:
    """
...
def run_backtest(strategy_class=None, initial_capital=None):
